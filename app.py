...
def find_question_type_and_create_answer(query, city, selected_time_type, selected_time):
    ...
    next_appearance_mode = bool(
        re.search("wann|zeitpunkt", query, re.IGNORECASE))
...
def query_processing(query):
    ...
    if cd.more_than_one_city() is True:
        ...
        if selected_time_type == "range":
            ...
            find_question_type_and_create_answer(
                query, city, selected_time_type, [range_start, range_end])
        if selected_time_type == "time_point":
            if td.check_if_time_point_can_be_looked_up(selected_time, None) is False:
                ...
                find_question_type_and_create_answer(
                    query, city, selected_time_type, [selected_time])
        if selected_time_type == "day":
            if td.check_if_day_is_one_of_the_next_14(selected_time, None) is False:
                print(
                    "Hoppla. Wir können für Sie nur Wetterinformationen für die nächsten 14 Tage bereitstellen.")
            else:
                find_question_type_and_create_answer(
                    query, city, selected_time_type, [selected_time])
...
def start_assistant():
   ...
   while True:
        ...
        print("value: ", mystdout.getvalue()[39:])
...
def run_dialog(query):
    old_stdout = sys.stdout
    sys.stdout = mystdout = StringIO()
    query_processing(query)
    sys.stdout = old_stdout
    return mystdout.getvalue()#[38:]

# print("value:",run_dialog("Wie wird das Wetter morgen?"))
# print("value:",run_dialog("Blödsinn"))
